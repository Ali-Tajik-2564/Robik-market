...
export default function CmsTopBar({ setOnSlideShow }) {
    ...
    useEffect(() => {
        if (windowWidth <= 1024) {
            Swal.fire({
                title: "به پنل مدیریت خوش امدید برای دسترسی به تمام صفحات روی پروفایل خود کلیک کنید"
                , icon: "success",
    ...
    return (
        <div className='w-full h-auto  sticky top-0 flex flex-row-reverse items-center justify-between px-4 p-3 mb-4'>
            <div className='flex flex-row-reverse items-center justify-center gap-3'>
                <img src=".././images/profile-1.png" className='w-14 h-14 rounded-full shadow-sm cursor-pointer' alt="" onClick={() => setOnSlideShow(true)} />
